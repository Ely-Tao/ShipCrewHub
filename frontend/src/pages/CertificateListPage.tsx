...
import { type Certificate, type CertificateStatus, type CertificateType } from '../types';
  ...
  const applyFilters = () => {
    ...
    if (filters.search) {
      filtered = filtered.filter(cert => 
        cert.certificate_number.toLowerCase().includes(filters.search.toLowerCase()) ||
        cert.issuing_authority.toLowerCase().includes(filters.search.toLowerCase())
      );
    }
    ...
    if (filters.certificateType) {
      filtered = filtered.filter(cert => cert.certificate_type === filters.certificateType);
    }
      ...
      <Card>
        <div style={{ marginBottom: 16 }}>
          <Row justify="space-between" align="middle">
            <Col>
              <Title level={4} style={{ margin: 0 }}>证书管理</Title>
            </Col>
            <Col>
              <Space>
                <Button
